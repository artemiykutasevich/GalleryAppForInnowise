...
extension Array where Element == UnsplashPageItem {
    func toPage() -> Page {
        self.compactMap({ $0.toPageItem() })
    }
}

extension UnsplashPageItem {
    func toPageItem() -> PageItem {
        return PageItem(
            id: id,
            isFavorite: false,
            createAt: createdAt.toDate() ?? Date(),
            size: CGSize(width: width, height: height),
            post: PageItemPost(title: altDescription, description: description),
            urls: PageItemURL(small: urls.small, regular: urls.regular)
        )
    }
}

/// typealias of `[UnsplashPageItem]`
...
//    let user: UnsplashUser
    ...
    enum CodingKeys: String, CodingKey {
        case id, urls, width, height, description
